...
function loadData(){
  ...
  .await(function(error,wave3,wave4,wave5,wave6){
    ...
    d3.select('svg g.chart')
      ...
      .attr('r', function(d) {
        //var ret = (isNaN(d[xAxis]) || isNaN(d[yAxis]) || isNaN(d[rAxis])) ? 0 : rScale(d[rAxis]);
    ...
    function updateChart(init) {
      ...
      d3.select('#rLabel')
        .text("● "+descriptions[rAxis]);
      // Update correlation
      var xArray = _.map(data[curWave], function(d) {return d[xAxis];});
      var yArray = _.map(data[curWave], function(d) {return d[yAxis];});
    ...
    function updateScales() {
      xScale = d3.scale.linear()
                      ...
                      .range([20, 900]);
      console.log(bounds[curWave][yAxis]);
...
//function loadData(){
  ...
  //.defer(d3.csv,'data/wave6.csv')
  //.defer(d3.csv,'data/wave5.csv')
  //.await(function(error,data6,data[0]){
    //if(error) {console.log(error);}
    //// Main part
    ...
    //var data= [parseData(data5),parseData(data6)];
    //var bounds = getBounds(data[0], 1);
    //console.log(bounds);
    //// SVG canvas
    //var svg = d3.select("#bubble-wrapper")
      //.append("svg")
    ...
    //function updateChart(init) {
      ...
      //d3.select('#rLabel')
        //.text("● "+descriptions[rAxis]);
      //// Update correlation
      //var xArray = _.map(data[0], function(d) {return d[xAxis];});
      //var yArray = _.map(data[0], function(d) {return d[yAxis];});
      //var c = getCorrelation(xArray, yArray);
      //var x1 = xScale.domain()[0], y1 = c.m * x1 + c.b;
      //var x2 = xScale.domain()[1], y2 = c.m * x2 + c.b;
